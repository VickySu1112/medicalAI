...
模型预测概率经 isotonic regression 单调化后按 25/50/75 % 分位切，得到 dev OOF 上 4 个 cuts: **0.263 / 0.332 / 0.406**（原概率尺度），再套 temporal。每档配独立的马卡龙色（凉色→暖色对应低→高风险）：Q1 薄荷绿、Q2 奶油黄、Q3 蜜桃粉、Q4 玫瑰红。

![图 11. M1 v6 — 4 档风险分层（isotonic-binned + 马卡龙配色，dev OOF 与 temporal 并列）。](figures/Figure_v3_11_FourTier_Macaron.png)
