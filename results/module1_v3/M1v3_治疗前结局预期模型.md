...
**9 个方法中 8 个与 L2-LR 在 paired bootstrap ΔAUC 上 95% CI 完全跨 0（统计等价），唯一显著低于 LR 的是 DecisionTree（Δ=−0.050★）；没有任何方法显著优于 LR**——这与 §4b.1 SHAP dependence 全直线 / §4b.2 PI 排序与 SHAP 高度一致 / §4b.3 PDP 全线性 三个独立分析的结论自然吻合：**M1 信号结构以线性可加为主，非线性方法没有可被开发的隐藏交互**。

### 6.8.2 双 split 性能与排名

| Rank | 方法 | **Dev OOF AUC** | **Temporal AUC** | Paired Δ vs LR (Temporal, 95% CI) |
|:---:|:---|:---:|:---:|:---:|
| #1 dev / #3 tmp | **L2-Logistic (ours)** | **0.723** (并列第 1) | 0.681 (0.602, 0.758) | (baseline) |
| 并列 #1 dev / #6 tmp | RandomForest | 0.723 (并列第 1) | 0.665 (0.579, 0.746) | −0.016 (−0.044, +0.014) |
| 并列 #1 dev / #5 tmp | SVM-RBF | 0.723 (并列第 1) | 0.674 (0.587, 0.753) | −0.007 (−0.041, +0.030) |
| #4 dev / #7 tmp | AdaBoost | 0.714 | 0.662 (0.577, 0.745) | −0.019 (−0.046, +0.005) |
| #5 dev / #9 tmp | DecisionTree | 0.703 | 0.631 (0.548, 0.713) | **−0.050★** (−0.091, −0.007) |
| #6 dev / #2 tmp | KNN | 0.702 | 0.683 (0.600, 0.763) | +0.002 (−0.039, +0.043) |
| #7 dev / #8 tmp | GradientBoosting | 0.699 | 0.650 (0.571, 0.732) | −0.031 (−0.070, +0.011) |
| #8 dev / #1 tmp | MLP | 0.688 | 0.688 (0.601, 0.766) | +0.006 (−0.029, +0.043) |
| #9 dev / #4 tmp | GaussianNB | 0.665 | 0.681 (0.596, 0.758) | −0.001 (−0.047, +0.049) |

★ = 95% CI 完全脱离 0（statistically loses to LR）。

### 6.8.3 双 split 排名差异的解读

L2-LR 在 **dev OOF 上点估计排第 1**（0.723，与 RF / SVM-RBF 并列），但在 **temporal test 上点估计排第 3**（0.681；与 MLP 0.688 差 0.006、与 KNN 0.683 差 0.002）。这一 cross-flip 看似矛盾，实质是 **N=201 temporal 上的采样波动**：

1. **前 4 名（MLP / KNN / LR / GaussianNB）的 temporal AUC 落在 0.681–0.688 的极窄带内**，差距 < 0.007 — 远小于 95% CI 宽度 0.16。paired bootstrap ΔAUC 对前 3 名都给 CI 完全跨 0 的判定（Δ < 0.01 / CI 跨 ±0.04）—— 统计上不可区分。
2. **Dev OOF 上 LR 居首**：在 802 例 training 数据上，LR 的 L2 收缩 + 线性可加假设使它的训练-验证泛化最稳；与 RF / SVM-RBF 并列第 1 (0.723)。同样模型在 temporal 上 LR 略掉到第 3，是 N=201 上 ±0.03 的方差，**不构成"非线性方法真的更优"的硬证据**。
3. **如果只看 temporal AUC 排名**，MLP / KNN / GaussianNB 略高于 LR；**如果只看 dev OOF 排名**，LR 与 RF / SVM 并列第 1。**两者放在一起 + paired ΔAUC CI 都跨 0 → 9 方法中前 4 名在 v6 6 特征上"统计等价"**。

### 6.8.4 与 §4b 五视角可解释性的串联

§4b.1 SHAP dependence 在 6 特征上呈精确直线 / §4b.2 PI 排序与 SHAP 在 Top 4 高度一致 / §4b.3 PDP + ICE 也全部直线 —— 这三个独立分析早就告诉我们：**M1 信号结构在 6 特征 v6 pool 上以线性可加为主，没有显著的非线性或交互**。本节 8 个非 LR 方法在 temporal 上**全部与 LR 统计等价**（含一个 DT 显著低于 LR），是这一结论在 model-agnostic ML 比较层面的独立确认。

### 6.8.5 L2-LR 的相对优势在性能之外

性能既然等档，方法选择就回到**可工程化属性**：
...
在所有 9 个方法中，**只有 L2-LR 同时具备这五个工程属性**。**性能等档 + 工程属性最优 → L2-LR 是临床部署的合理选择**。
