...
文献里常用的 8 种机器学习方法构成临床预测研究的"必报"对比矩阵：RandomForest、GradientBoosting、AdaBoost、DecisionTree、KNN、SVM-RBF、GaussianNB、MLP。我们在**完全相同的 v6 6 特征 pool、完全相同的训练/temporal 切分、完全相同的 Platt-校准外包装**下，对这 8 个方法 + L2-LR baseline 做了头对头比较。

![图 30. L2-LR 与 8 个常见 ML 方法在同一 v6 6 特征 pool 上的 temporal-test AUC 对比。](figures/Figure_v3_30_ML_Comparison.png)

| 排名 | 方法 | 归纳偏置 | Temporal AUC (95% CI) | 与 LR 差 (paired Δ, 95% CI) |
|:---:|:---|:---|:---:|:---:|
| 1 | MLP (64-32, ReLU) | feedforward neural network | 0.688 (0.601, 0.766) | +0.006 (−0.029, +0.043) |
| 2 | KNN (k=25, distance) | instance-based local lookup | 0.683 (0.600, 0.763) | +0.002 (−0.039, +0.043) |
| **3** | **L2-Logistic (ours)** | **linear + L2 + Platt** | **0.681 (0.602, 0.758)** | **(baseline)** |
| 4 | GaussianNB | generative, marginal independence | 0.681 (0.596, 0.758) | −0.001 (−0.047, +0.049) |
| 5 | SVM-RBF (C=1, γ='scale') | kernel margin | 0.674 (0.587, 0.753) | −0.007 (−0.041, +0.030) |
| 6 | RandomForest (300 trees, depth 6) | bagged trees, axis-aligned splits | 0.665 (0.579, 0.746) | −0.016 (−0.044, +0.014) |
| 7 | AdaBoost (200 stumps, lr 0.5) | boosting on decision stumps | 0.662 (0.577, 0.745) | −0.019 (−0.046, +0.005) |
| 8 | GradientBoosting (200 trees, depth 3) | boosting on residual gradient | 0.650 (0.571, 0.732) | −0.031 (−0.070, +0.011) |
| 9 | DecisionTree (depth 5) | single CART tree | 0.631 (0.548, 0.713) | **−0.050★ (−0.091, −0.007)** |

★ = 95% CI 完全脱离 0（statistically loses to LR）。

**核心观察**：

1. **L2-LR 在 9 方法中点估计排第 3**（与第 1 MLP 差 0.006、与第 2 KNN 差 0.002、与第 4 GaussianNB 差 0.000）——前 4 名（含 LR）的 temporal AUC 在 0.681–0.688 的极窄带内，paired bootstrap Δ CI 全部跨 0：**统计上完全不可区分**。8/9 方法与 LR 等价；唯一显著输给 LR 的是 DecisionTree（−0.050，CI 完全脱 0）。
2. **CI 重叠极大 → 没有任何方法显著优于 LR**——所有 9 个方法的 temporal 95% CI 都包含 0.65–0.77 这一带；paired Δ CI 也都跨 0（除 DT）。这意味着**用更复杂的非线性方法替代 LR 没有可证明的性能收益**，但代价是失去 Wald CI 的闭式可解释性、Platt-校准的概率质量优势、以及个体特征贡献的精确分解。
3. **L2-LR 的相对优势在性能之外**——简约（6 个特征）+ 可解释（标准化系数 → OR + 95% CI 闭式精确）+ 校准良好（slope 0.90, intercept ≈ 0）+ 模型公式可写出（§2.5）+ 推理速度可忽略。在所有 9 个方法中，只有 LR 同时具备这五个工程属性。**性能等档但工程属性最优 → LR 是临床部署的合理选择**。
4. **Dev OOF 与 Temporal 之间的 cross-flip**——LR 在 dev OOF 上 AUC 0.7233（与 RF 0.7230 / SVM 0.7230 并列第 1），但在 temporal 上掉到第 3；MLP 与 KNN 在 dev OOF 上较低（0.6883 / 0.7024）但 temporal 上反超 LR。在 N=201 的 temporal 上这种排名轻微交叉是采样波动可预期的结果，不构成"非线性方法更优"的硬证据——前 4 名的 paired Δ CI 全部跨 0 就是直接说明。

**方法学引用**：本节实验脚本 `scripts/simple/module1_v3_ml_compare_v6_fullfit.py`；汇总表 `results/module1_v3/tables/ml_comparison_table.csv`；paired Δ 表 `results/module1_v3/tables/ml_comparison_deltas.csv`；汇总 JSON `results/module1_v3/tables/ml_comparison_summary.json`。
