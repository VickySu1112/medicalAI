...
**(a) `f(甲状腺重量)` @0M —— S 形阈值**:小腺体降风险、30–100 g 陡升、>100 g 饱和于 +1.6。LR 只能给一条斜直线,给不出"先平、再陡、后饱和"。

![甲状腺重量形状函数](m2v2_ebm_full/figures/F05_Shape_0M_r1.png)

**(b) `f(FT3,FT4 综合变化速度)` @6M —— 过零陡降(动量拐折)**:速度为负(激素没回落)贡献 +1.8,**过零点骤降**,速度为正后 −0.65。拐折点正好在零——这是 LR 的单一斜率根本无法表达的非线性,也是本项目最有临床价值的发现。

![速度形状函数](m2v2_ebm_full/figures/F32_Shape_6M_r1.png)

> 对应图谱:`f(甲状腺重量)` = F05/F14/F24;`f(FT3,FT4 综合水平)` = F15/F23;`f(速度)` = F32;`f(TSH 速度)` = F33;并排面板 = F44。交互版里每地标可 hover 的 8 条曲线全是 `f`。
