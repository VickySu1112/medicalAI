...
下图是 **6M EBM 真实学到的** `g(TPOAb, FT3,FT4 落差)`(corrected+LOCF 口径下,它是 6M 首位交互,importance≈0.24):左 = 原始 62×62 网格(每像素一个独立 log-odds 值)+ 训练样本黑点;右 = 同表粗化成 5×5 便于读数。(0M 不再做 EBM——由 M1 承担;此处用同一特征对在 6M 演示,教学点不变、口径已更正。)

![2D 交互查表](m2v2_ebm_full_locf/figures/Interaction_demo_6M.png)
...
**"耦合"是什么?** 看右图 5×5:**最上一行(落差为正/高)**,TPOAb 从低→高,贡献从 **+0.07 翻成 −0.34**(升险→降险);**最下一行(落差为负)** 则相反,TPOAb 从低→高,贡献从 **−0.40 翻成 +0.40**。**一个指标的效应方向,取决于另一个指标当下的取值**——这正是可加模型 `f_i+f_j` 永远做不到的。
...
**"不假设正交"不等于"相关没后果":** 上图(6M)TPOAb 与落差 `corr=+0.03`(几乎不相关),可 62×62 网格**只有 14.8% 的格子真有样本**(黑点),其余 85% 的 `g` 是正则化外推的、不可信(置信带宽)。后果有二:
...
### 6.1 定位与继承
M2 = "治疗后早期长期风险更新":在 **1/3/6/12M** 四个固定地标各训一个 EBM(0M 由 M1 承担),回答"3 个月 / 6 个月 / 12 个月时还要不要担心 24 个月以后"。它**继承 M1** 的基线标量风险作为一个特征,并为 **M3 滚动地标**提供"当前状态 + 动量"的特征模板。

### 6.2 重要性随时间迁移(EBM 的招牌发现 + 口径修订)
原生重要性(组×地标)清楚显示主导特征**换人**。**corrected 真值后**:甲状腺重量(1M)→ **「FT3,FT4 综合水平」(3M 接管,并在 6M / 12M 持续主导)**;当期 TSH 自 3M 稳居第二梯队;**激素动量(FT3,FT4 综合变化速度)仅 6M / 12M 中等(退居第三、第五)**。即修订后的轨迹是"静态解剖负荷(1M)→ **当期激素水平(3M 起主导至 12M),动量为辅**"。

![重要性时间迁移](m2v2_ebm_full_locf/figures/F41_GroupImportance_heatmap.png)

> **这是本项目最重要的口径修订。** ~~旧退化口径下的迁移叙事为"解剖负荷 → 当期激素水平 → 激素动量(FT3,FT4 速度 + TSH 速度,6M 居首)"~~——但那源自主线读退化宽列(`FT4_6M` 仅 13/1003 真值、`FT4_12M` 0/1003)、当期化验被抹平所致的**假象**。修复"读错当期化验列"后,**当期真实甲功的强信号回归并主导后期**;激素动量并非不重要——它以 **12M「水平 × 速度」交互**(importance 0.37,进 top-3)和 6M「当期 TSH × 速度」(0.23)的形式提供"在当前状态之上的增量"(momentum-beyond-inertia),而非作为独立主效应主导 6M。

### 6.3 与朴素基线同台:persistence(当前功能态)+ naive(prevalence)
本版以两条朴素对照锚定学习模型的增量:**naive**(prevalence,AUC 0.5)与 **persistence**(只看当期 TSH 的 L2-LR,即"当前功能态会持续")。逐地标 EBM 稳定高出 persistence **0.13–0.16**(1M 0.694 vs 0.55 / 3M 0.791 vs 0.68 / 6M 0.878 vs 0.75 / 12M 0.908 vs 0.75);persistence 升到 0.75 后封顶,而 EBM 仍随随访继续抬升——**学习模型相对"只读今天甲功"的增量真实且稳定,12M 最大**。结合"随机森林可把训练 AUC 拟合到 1.000 而泛化不超 EBM、学习曲线半量即饱和",任务仍处**特征信息天花板**:复杂度堆不出上限,EBM 的价值在解释 + 朴素锚定,而非黑盒判别增益。

![EBM vs persistence / naive](m2v2_ebm_full_locf/figures/F38_EBM_vs_persistence.png)

### 6.4 选择性预测(部署可靠性)
让模型对没把握的病例"不下结论、转交医生":按不确定性弃权 0→50% 时,保留人群的准确率与 NPV 两线全程上行无回撤。适合"高置信自动、低置信转人工"的分诊;per-landmark 看,12M 弃 30% 可把 acc 抬到约 0.90、NPV 约 0.91,而 1M 早期弱信号弃权救不回。

![选择性预测](m2v2_ebm_full_locf/figures/F45_Selective_prediction.png)

### 6.5 交互式可视化
[Module2v2_EBM_interactive.html](Module2v2_EBM_interactive.html) —— plotly 自包含单页(corrected+LOCF,地标 1/3/6/12):每地标的形状放大镜(下拉选特征,hover 读 log-odds + bagging 置信带、滑块缩放)+ 重要性条形;**12M 交互项 2D 查表探索**(下拉换交互项、悬停看每格人次/事件率,首项即「当期甲功水平 × 其变化速度」)+ 6M / 12M 单病人 waterfall。浏览器直接打开即可拨弄。
...
### 附录 · 环境 / 数据 / 合规 / 关联
- 环境:本机 `.venvs/rai_b4`(interpret-core 0.7.8 / sklearn 1.8.0 / plotly),`PYTHONNOUSERSITE=1 OMP_NUM_THREADS=1 MKL_NUM_THREADS=1`;数据自计算机 rsync。
- 口径:corrected 真值(`Current_Time` 列)+ LOCF(激素延续,time-safe 仅回看),地标 1/3/6/12;EBM 逐地标 dev 5 折 StratifiedGroupKFold OOF(选择)+ 时间外(报告)。
