...
| **M2** | 早期固定地标长期风险更新（1/3/6/12M;0M 由 M1 承担）：回答"3/6/12 个月时还要不要担心 24 个月以后"。**主线 = EBM 玻璃盒**（逐地标可解释 GAM）——揭示**重要性时间迁移**（甲状腺重 → FT3,FT4 综合水平@3/6/12M → 综合变化速度 / TSH 反馈状态于后期）;偏差-方差 + 学习曲线确证**特征天花板**（RF train 1.0 / OOF=LR、半量即饱和）,故主打**可解释 + 选择性预测**（弃权 50% → 准确率 0.78 / NPV 0.84）。**口径(2026-06 更新)**:主线插值 = **median**(传统硬插中位数);Temporal AUC 1M 0.702 / 3M 0.832 / **6M 0.858** / **12M 0.795** —— **12M < 6M 符合"lead-time 越短 AUC 越高"的物理直觉**(LOCF 12M=0.908 高出 +0.105,主要来自 6M 真值结转给 12M 那 398 个脱落者,改为敏感性对照)。**EBM 论文 md/html 每图后追加 47 段「临床读法」**(rule-out/rule-in 阈值取舍 → 该工作点 PPV/NPV 给患者的把握度;校准点能否直接告知概率;每次复诊看哪个特征;low 档 NPV ≥ 0.90 能否降级随访)。方法学方框:serial-landmark supermodel（Pooled ROC 0.770）/ 5-法 benchmark（L2·Elastic-net·GEE·RF·HGB）/ 3 架构（MDJN·Dual-Tower·CLAN）/ B4 GRU·GRU-ODE / Shapley + 2×2 head-to-head;per-landmark 4-LR + Eval state（v1）作 fallback。 | **[EBM 玻璃盒论文·图文版（47 图 + 47 段逐图临床读法 · median 主线 · 12M < 6M 物理直觉，★ 推荐入口）](results/module2_v2_vertical/Module2v2_EBM_paper.md)**<br>· 入门 [原理图解教程](results/module2_v2_vertical/Module2v2_EBM_讲解.md) · 可玩 [交互版·plotly](results/module2_v2_vertical/Module2v2_EBM_interactive.html)<br>· [三轨综合 + 推荐](results/module2_v2_synthesis/Module2v2_三轨综合与论文推荐.md) · [probe 诊断](results/module2_v2_vertical/Module2v2_probe_可解释性与诊断.md) · [12 方法对比](results/module2_v2_vertical/Module2v2_compare.md)<br>· [M2·v1 fallback](results/module2_early_landmark_updating/Module2_早期固定地标长期风险更新.md)<br>· **median 47 图 atlas**:[`m2v2_ebm_full_median/`](results/module2_v2_vertical/m2v2_ebm_full_median/)(主线产物 · 47 PNG + manifest.json + ebm_median_metrics.json) | [`results/module2_v2_base/`](results/module2_v2_base/) · [`results/module2_v2_horizontal/`](results/module2_v2_horizontal/) · [`results/module2_v2_vertical/`](results/module2_v2_vertical/) · [`results/module2_v2_synthesis/`](results/module2_v2_synthesis/) · [`results/module2_early_landmark_updating/`](results/module2_early_landmark_updating/) |
...
- **可解释性与特征天花板深化（v2 续作）**：B4 时间感知 GRU / GRU-ODE、偏差-方差 + 学习曲线诊断、三模型一致的**重要性时间迁移**、**选择性预测**(弃权 50% → 0.78 / NPV 0.84)、2D 交互查表与正交/共线性辨析 —— **报告入口(EBM 图解教程 / 图文论文 / 交互版 / probe / 12 方法对比)见上方表格 M2「中文报告」列**。
- **EBM 玻璃盒论文新增内容（2026-06）**:① 4 时点扩到 **1/3/6/12M**(0M 由 M1 承担,本文不再做 0M 性能分析);② 主线插值切换 **LOCF → median**(传统硬插中位数),Temporal AUC `1M 0.702 · 3M 0.832 · 6M 0.858 · 12M 0.795`,**12M < 6M 符合 lead-time 越短 AUC 越高的物理直觉**(LOCF 12M=0.908 高出 +0.105 主要来自 6M 真值结转给 12M 那 398 个脱落者,改为敏感性对照而非主线);③ 每张图后追加 blockquote `> **临床读法**:...`(rule-out/rule-in 阈值取舍 · PPV/NPV 给患者的把握度 · 概率能否直接告知 · 每次复诊看哪个特征 · low 档 NPV ≥ 0.90 能否降级随访),**47 图 47 段全程覆盖**。
