...
Fork of module2_v2_ebm_atlas.py re-cut for the FINAL口径:
  * data   : module2_v2_impute_experiment.build_rows_for_method("median", …)
...
Output: results/module2_v2_vertical/m2v2_ebm_full_median/figures/ + manifest.json.
"""
...
LANDMARKS = (1, 3, 6, 12)
OUT = ROOT / "results" / "module2_v2_vertical" / "m2v2_ebm_full_median"
FIGD = OUT / "figures"
...
def main() -> None:
    ...
    rows = build_rows_for_method("median", landmarks)
    ...
    metrics_dump = {
        "口径": "corrected truth (Current_Time) + median impute (time-safe), landmarks 1/3/6/12",
        "per_landmark": {
            f"{L}M": {
    ...
    (OUT / "ebm_median_metrics.json").write_text(json.dumps(metrics_dump, indent=2, ensure_ascii=False))
