...
![图 1. LASSO 选择路径：5 折平均非零系数随 C 变化。]({fig_path("Figure_01_LASSO_Path.png")})
...
![图 2. Temporal-test ROC 与 PR 曲线。]({fig_path("Figure_02_ROC_PR_core_vs_augmented.png")})

**图 2 解读**：ROC 曲线在 temporal test 上 AUC ≈ {fmt(perf.loc[('Temporal_Test', 'ROC_AUC')]['Value'])}, PR 曲线远高于 0.408 的患病率参考线。10 特征 LASSO 简约模型在 development OOF 与 temporal test 上的性能数字非常接近，说明模型在时间外验证上无明显退化。
"""
