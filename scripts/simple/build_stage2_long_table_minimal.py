...
def main() -> None:
    ...
    print(f"  built {len(out)} rows × {out.shape[1]} cols", flush=True)
    # Sanity: every landmark FT4 coverage
    for L in (0, 1, 3, 6, 12, 18, 24):
        cov = out[f"FT4_{L}M"].notna().mean() * 100
        print(f"  FT4_{L}M non-null: {cov:.1f}%", flush=True)
