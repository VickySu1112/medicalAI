...
**本版统一 corrected 真值 + LOCF(激素延续)口径,地标 1/3/6/12**(与图文版 paper
/ atlas_locf 同源)。每地标一个 EBM,dev 5 折 StratifiedGroupKFold OOF + 时间外由
final dev-fit 预测(`ebm_oof_and_temporal`);AUC 报时间外。
...
def _resolve(term: str, live) -> str:
    ...
    ebm_oof_and_temporal 在 numpy array 上 fit,故 explain_global() / term_names_
...
<b>本版口径</b>:corrected 真值(<code>Current_Time</code> 列)+ LOCF(激素延续),地标 <b>1/3/6/12</b>(0M 由 M1 承担)。时间外 ROC-AUC 1M 0.694 / 3M 0.791 / 6M <b>0.878</b> / 12M <b>0.908</b>。<b>重要性叙事修订:当期甲功水平(FT3,FT4 综合水平)自 3M 接管首位并主导 6M / 12M;激素动量退居 6M 第三、12M 第五,以「水平 × 速度」交互形式提供增量</b>(旧退化口径“动量主导 6M”系当期化验读退化宽列所致,已更正)。<br>
<span class="note">分析单元 = 治疗疗程(N=1003 人次);dev 5 折 StratifiedGroupKFold OOF,时间外集报告 AUC(EBM final dev-fit 预测);置信带来自 EBM 的 bagging。静态论文版见 Module2v2_EBM_paper(.html),原理见 Module2v2_EBM_讲解(.html)。</span>
</div>
"""
TAIL = "</body></html>"
...
def main():
    ...
    for L in LANDMARKS:
        ...
        # EBM OOF(dev) + temporal(final dev-fit);返回 final dev-fit EBM + live 列
