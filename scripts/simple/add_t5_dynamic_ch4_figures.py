...
def topk_ci_tables(pred: pd.DataFrame, patient: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    patient_frame = (
        ...
        .rename(columns={"Patient_Event": "Y_Relapse", "Max_Interval_Risk": "DirectProb"})
...
def patient_topk_capture_curve(patient: pd.DataFrame) -> pd.DataFrame:
    ...
    model = patient[patient["Split"].eq("TemporalTest")][["Patient_ID", "Patient_Event", "Max_Interval_Risk"]].copy()
    ...
    merged = merged.rename(columns={"Max_Interval_Risk": "FourBranchMaxRisk", "Patient_Event": "Y"})
    n = len(merged)
    total_events = max(1, int(merged["Y"].sum()))
    rows = []
    for k in range(5, 101, 5):
        top_n = max(1, int(np.ceil(n * k / 100.0)))
        top_model = merged.sort_values("FourBranchMaxRisk", ascending=False).head(top_n)
    ...
    ax.plot(curve["TopK_Percent"], curve["FourBranch_Capture_Rate"], marker="o", color="#111827", label="4branch max-risk")
...
def patient_cumulative_table(pred: pd.DataFrame) -> pd.DataFrame:
        ...
        rows.append(
            {
                ...
                "Cumulative_Risk": float(1 - np.prod(1 - np.clip(probs, 0, 1))),
    ...
    out.to_csv(TAB / "readme4_patient_cumulative_risk.csv", index=False)
    return out


def q1q4_summary(patient: pd.DataFrame) -> pd.DataFrame:
    test = patient[patient["Split"].eq("TemporalTest")].copy()
    test["Risk_Quartile"] = pd.qcut(test["Cumulative_Risk"], q=4, labels=["Q1", "Q2", "Q3", "Q4"], duplicates="drop")
    rows = []
    for q, part in test.groupby("Risk_Quartile", observed=False):
        n = int(len(part))
        ...
        rows.append(
            {
                ...
                "Mean_Cumulative_Risk": float(part["Cumulative_Risk"].mean()),
                "Mean_Max_Interval_Risk": float(part["Max_Interval_Risk"].mean()),
                "Wilson_CI_Low": low,
                "Wilson_CI_High": high,
                "Risk_Min": float(part["Cumulative_Risk"].min()),
                "Risk_Max": float(part["Cumulative_Risk"].max()),
            }
        )
    out = pd.DataFrame(rows)
    out.to_csv(TAB / "readme4_patient_q1q4_cumulative.csv", index=False)
    return out
...
def fig_q1q4(qsum: pd.DataFrame) -> None:
    ...
    y = qsum["Observed_Event_Rate"].to_numpy()
    yerr = np.vstack([y - qsum["Wilson_CI_Low"].to_numpy(), qsum["Wilson_CI_High"].to_numpy() - y])
    ax.bar(x, y, yerr=yerr, capsize=4, color="#9CC7E6", edgecolor="#2F5F7F", label="Observed event rate")
    ax.plot(x, qsum["Mean_Cumulative_Risk"], marker="o", color="#D98C2B", lw=2.2, label="Mean cumulative predicted risk")
    for i, r in qsum.iterrows():
        ax.text(i, y[i] + 0.035, f"{int(r['Events'])}/{int(r['N'])}", ha="center", fontsize=9)
    ax.set_xticks(x, qsum["Risk_Quartile"])
    ax.set_ylim(0, max(1.0, float(qsum["Mean_Cumulative_Risk"].max()) + 0.08))
    ax.set_ylabel("Patient-level event rate / risk")
    ax.set_title("Temporal test patient-level Q1-Q4 risk strata")
...
def fig_waterfall(patient: pd.DataFrame) -> None:
    test = patient[patient["Split"].eq("TemporalTest")].sort_values("Cumulative_Risk", ascending=False).reset_index(drop=True)
    risks = test["Cumulative_Risk"].astype(float).to_numpy()
    cmap = RISK_CMAP
    norm = RISK_NORM
    ...
    ax.set_xlabel("Temporal test patients sorted by cumulative risk")
    ax.set_ylabel("1 - product(1 - interval risk)")
    ax.set_title("Patient-level cumulative risk waterfall")
    ax.set_ylim(0, min(1.02, max(0.2, float(test["Cumulative_Risk"].max()) + 0.08)))
    ...
    cbar.set_label("Cumulative risk")
...
def fig_heatmap(pred: pd.DataFrame, patient: pd.DataFrame) -> None:
    ...
    patient_order = patient[patient["Split"].eq("TemporalTest")].sort_values(["Patient_Event", "Cumulative_Risk"], ascending=[False, False])
...
def choose_typical_patients(patient: pd.DataFrame) -> pd.DataFrame:
    ...
    if len(eligible) < 3:
        eligible = test
    event = eligible[eligible["Patient_Event"].eq(1)].sort_values(["Cumulative_Risk", "N_Intervals"], ascending=[False, False]).head(1)
    low = eligible[eligible["Patient_Event"].eq(0)].sort_values(["Cumulative_Risk", "N_Intervals"], ascending=[True, False]).head(1)
    ...
    adjacent = adjacent_pool.iloc[(adjacent_pool["Cumulative_Risk"] - THRESHOLD).abs().argsort()[:1]]
    out = pd.concat(
        [
...
def fig_typical(pred: pd.DataFrame, err: pd.DataFrame, chosen: pd.DataFrame) -> None:
    ...
    for ax, (_, row) in zip(axes, chosen.iterrows()):
        ...
        ax.set_ylim(0, 1.02)
        ax.set_title(f"{row['Case_Label']}: {row['Case_Type']}\ncumulative risk={row['Cumulative_Risk']:.2f}")
...
def update_readme(qsum: pd.DataFrame) -> None:
    q_brief = ", ".join([f"{r.Risk_Quartile}: {int(r.Events)}/{int(r.N)}" for r in qsum.itertuples()])
    block = f"""{START}
...
Figure 17 将 temporal test 患者按累计风险 `1 - product(1 - p_t)` 分为 Q1-Q4。柱为观察到的患者级复发率，误差条为 Wilson 95% CI，橙线为组内平均累计预测风险。当前四分位事件数为：{q_brief}。
...
Figure 19 将 temporal test 的 patient-interval 风险矩阵化显示：列是随访窗口，行是患者，颜色是该 interval 的复发风险。复发患者和未复发患者分面展示，并在各自面板内按累计风险排序。
...
{HISTORY_END}"""
    ...
    additions = {
        ...
        "- `tables/topk_patient_capture_ci.csv`：patient-level max-risk Top-K capture 及 Patient_ID cluster bootstrap 95% CI。": "- `tables/patient_level_maxrisk_warning.csv`：patient-level max-risk 预警。",
...
def main() -> None:
    ...
    patient = patient_cumulative_table(patient_pred)
    qsum = q1q4_summary(patient)
    topk_interval_ci, _ = topk_ci_tables(pred, patient)
