...
换成 matplotlib 静态图:
  ...
  图1【最重要特征 3D 曲面】12M「激素负荷 Hormone_load × 动量负荷 Velocity_load」
       ...
       → results/module2_v2_vertical/m2v2_bin16_top_interaction_3d.png
  ...
  图2【降维整体风险景观】6M PCA PC1×PC2 散点 + dev OOF 概率插值决策面(contourf,
       ...
       → results/module2_v2_vertical/m2v2_bin16_pca_landscape.png

口径(硬约束):
  - 数据 corrected + LOCF:`build_rows_for_method("locf", (1,3,6,12))`。
...
def build_interaction(L: int, prefset: set[str]) -> dict:
    ...
    """
    rows = build_rows_for_method("locf", (1, 3, 6, 12))
...
def fig_interaction_3d(out_path: Path) -> dict:
    ...
    ax.set_title(
        f"EBM 交互项 3D 决策曲面 · {L}M「{D['a']} × {D['b']}」(bin16,GA2M 自动 #1 交互)\n"
    ...
    fig.text(
        0.015, 0.015,
        f"momentum-beyond-inertia 的交互增量 · corrected+LOCF · 分析单元 1003 人次 · "
        f"importance={D['imp']:.4f} · |g|max={vmax:.2f} · "
        f"(该命名交互在 12M 未被 GA2M 自动选入,故取其自动 #1 的 {L}M)",
...
def fig_pca_landscape(out_path: Path, L: int = 6) -> dict:
    ...
    fig.text(
        0.015, 0.012,
        f"corrected+LOCF · 分析单元 1003 人次 · n@{L}M={d['n_at']}"
...
def main() -> None:
    ...
    if args.only in (None, "3d"):
        p1 = OUT / "m2v2_bin16_top_interaction_3d.png"
    ...
    if args.only in (None, "pca"):
        p2 = OUT / "m2v2_bin16_pca_landscape.png"
        info2 = fig_pca_landscape(p2, L=6)
